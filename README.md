...
개발자가 아니면 `git clone`이나 터미널 명령어를 사용하지 않아도 됩니다.

1. [프로그램 ZIP 다운로드](https://github.com/kangjung/kakaowork-backup/archive/refs/heads/main.zip)를 누릅니다.
2. 내려받은 `kakaowork-backup-main.zip` 파일을 마우스 오른쪽 버튼으로 클릭하고 **압축 풀기**를 선택합니다.
3. 압축을 푼 폴더 안에서 `Run-Exporter.cmd`를 더블 클릭합니다.
4. 카카오워크에서 백업할 대화방을 열어 둔 뒤, 프로그램 화면의 **반출 시작** 버튼을 누릅니다.
5. 여러 대화방 백업을 한 번에 보려면 `Open-BackupViewer.cmd`를 더블 클릭하고 백업 폴더를 선택합니다.

자세한 화면별 안내는 [비개발자용 사용 튜토리얼](docs/non-developer-tutorial.md)을 참고하세요.

![카카오워크 대화 반출 실행 화면](docs/images/exporter-ui.png)

## 목차

- [처음 사용하는 분은 여기부터](#처음-사용하는-분은-여기부터)
- [주요 기능](#주요-기능)
- [동작 원리](#동작-원리)
- [요구 사항](#요구-사항)
- [빠른 시작](#빠른-시작)
- [최대 스크롤 횟수 선택](#최대-스크롤-횟수-선택)
- [여러 대화방 한 번에 보기](#여러-대화방-한-번에-보기)
...
개발자가 아니라면 위의 [처음 사용하는 분은 여기부터](#처음-사용하는-분은-여기부터)를 먼저 보세요.
아래 명령어는 Git을 사용할 줄 아는 개발자용입니다.
...
6. **최대 스크롤 횟수**를 확인합니다. 처음에는 기본값 `500`을 권장합니다.
7. **반출 시작** 버튼을 누릅니다.
8. 반출 중에는 카카오워크가 스크롤할 때마다 화면 맨 앞으로 올라오므로
   **실행 중에는 컴퓨터로 다른 작업을 할 수 없습니다.** 자리를 비울 때
   실행하는 것이 좋습니다. 카카오워크 창을 직접 조작하거나 다른 대화방으로
   이동하지 말고, **최소화하지 마세요.**
9. 중간에 멈추려면 **중지** 버튼을 누릅니다. 지금까지 수집한 내용으로 HTML ·
   JSON이 저장됩니다. (이어하기는 지원하지 않으므로, 다시 실행하면 최신
   메시지부터 처음부터 다시 수집합니다.)
10. 지정한 저장 폴더에서 결과를 확인합니다.

기본 저장 폴더 `exports`는 프로그램 폴더 안에 생성됩니다.

## 최대 스크롤 횟수 선택

**최대 스크롤 횟수**는 카카오워크 대화방을 위로 몇 번까지 넘겨 볼지 정하는 **상한선**입니다.
대화방의 처음까지 도달하면 이 숫자를 다 채우지 않아도 자동으로 멈춥니다.
값을 크게 잡아도 끝까지 가면 알아서 종료되지만, 대화량이 아주 많은 방에서는 실행 시간이 길어질 수 있습니다.

| 상황 | 권장 값 |
| --- | --- |
| 최근 며칠 또는 메시지가 적은 대화방 | `100` ~ `300` |
| 일반적인 업무 대화방 | `500` |
| 몇 달 이상 쌓인 대화방 | `1000` 이상 |
| 아주 오래된 대화까지 최대한 저장해야 함 | `2000` 이상, 시간이 오래 걸릴 수 있음 |

얼마로 해야 할지 모르겠거나 전체 대화를 최대한 저장하고 싶다면 값을 넉넉하게 잡아도 됩니다.
다만 너무 오래 걸리는 것을 피하고 싶을 때 위 표를 참고하세요. 결과가 원하는 기간보다 짧으면
값을 더 크게 올려 다시 실행하면 됩니다. 너무 오래 걸리면 **중지** 버튼을 눌러 지금까지
수집한 내용만 저장할 수 있습니다.

## 여러 대화방 한 번에 보기

여러 대화방을 각각 반출한 뒤에는 `Open-BackupViewer.cmd`를 더블 클릭해 통합 뷰어를 열 수 있습니다.
뷰어는 브라우저에서 실행되는 로컬 HTML이며, 사용자가 선택한 폴더 안의 백업 JSON과 이미지 폴더를 읽습니다.

![카카오워크 통합 백업 뷰어](docs/images/backup-viewer-ui.png)

1. `Open-BackupViewer.cmd`를 더블 클릭합니다.
2. 왼쪽 채팅방 목록 아래의 **백업 폴더 선택**을 누릅니다.
3. `exports` 폴더, 프로젝트 폴더, 또는 여러 백업 폴더를 모아 둔 상위 폴더를 선택합니다.
4. 왼쪽 채팅방 목록에서 대화방을 고릅니다.
5. 상단에서 **현재 방** 또는 **전체** 검색 범위를 선택합니다.
6. 넓은 검색창에 대화방 이름, 발신자, 메시지 본문을 입력한 뒤 **검색**을 누르거나 Enter를 누릅니다.
7. 왼쪽 검색 결과를 누르면 해당 대화방으로 이동하고, 오른쪽 타임라인의 해당 메시지 위치가 강조됩니다.

채팅방 목록은 왼쪽 채팅 아이콘이나 목록 상단 버튼으로 접고 펼칠 수 있습니다.
검색 결과 패널과 대화 타임라인도 각 패널 제목 오른쪽의 원형 버튼으로 접고 펼칠 수 있습니다.
뷰어는 발신자 이름이 없는 메시지를 내가 보낸 메시지로 보고 오른쪽에 표시합니다.
발신자 이름이 있는 메시지는 다른 사람이 보낸 메시지로 보고 왼쪽에 표시합니다.
브라우저 보안상 뷰어가 PC의 폴더를 자동으로 훑지는 못합니다. 반드시 사용자가 폴더를 직접 선택해야 합니다.
이미지는 JSON 파일 이름과 같은 `_images` 폴더가 함께 선택된 폴더 안에 있을 때 표시됩니다.
Chrome 또는 Edge에서 **백업 폴더 선택**을 사용하면, 브라우저 권한이 유지되는 동안 다음 실행 시 같은 폴더를 자동으로 다시 읽습니다.
자세한 화면별 안내는 [통합 백업 뷰어 사용법](docs/backup-viewer.md)을 참고하세요.

> [!TIP]
> 터미널에서 실행한 경우 `Ctrl + C`로 중단할 수 있습니다. 다만 이 경우
> 그때까지의 데이터는 체크포인트 파일에만 남고 HTML · JSON은 생성되지 않습니다.
> 정상적으로 마무리하며 멈추려면 저장 폴더에 `_stop_requested.flag` 파일을
> 만들면 됩니다(HTA의 **중지** 버튼이 하는 일과 동일).
...
| `-MaximumScrolls` | `500` | 과거 메시지를 탐색할 최대 스크롤 횟수. 클수록 더 오래 탐색하지만 실행 시간이 길어짐 |
| `-ScrollDelayMilliseconds` | `700` | 스크롤 후 화면이 그려질 때까지 대기 시간. 메시지가 누락되면 늘리세요 |
